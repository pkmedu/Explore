...
libname sds 'C:\Data\SDS_Folder';    /* Assign libref to the output SAS data set */
...
proc sort data=PSAQ_Frame_2013; 
 by dupersid; 
run;
proc sort data=SDS.h173 out=work.PSAQ_data;  /* Create a temporary sorted data set */
   by dupersid; 
run;

* Create a PSAQ analytic data file by mergeing the PSAQ data file with the FRAME data file;

data sds.MatchMergePSAQ;
  ...
  * Create a Smoke_Every_day variable;
   ...
   label r_racethx2 = 'Race/Ethnicity Recode'
                r_edrecode2 = 'Education Recode'
				marital_s = 'Marital Status'
                d_famscat = 'Family Size'
                r_povcat =  'Federal Poverty Levels Recode'
                empstat13 = 'Employment Status Recode'
                r_region = 'Geographic Region'
                health = 'Perceived Health Status Recode'
                obv10_plus = 'Office-Based Provider Visits (12-Month Period) Recode'
                x_psaqage = 'PSAQ Respondent Age'
				x_flushot = 'Whether Took Flu Shot (12-Month Period) Recode'
                x_ofttobac = 'Whether Smoke Everyday in Past Year Recode';
  if a=b;     /* matches only */
run;

title '2014 PSAQ Analytic File - Frequency Tables for Selected Variables';
proc freq data=sds.MatchMergePSAQ;
...
proc means data=sds.MatchMergePSAQ N min max maxdec=0; 
run;
