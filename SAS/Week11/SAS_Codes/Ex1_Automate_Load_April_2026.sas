...
************************************************************************/
DM "log; clear; output; clear; odsresults; clear;"; /* Clear log, output, and ODSresults */
...
/***************************************************************************
* Before assigning the concatenated library, check whether each of the four * 
* folders exist by wrapping the LIBNAME statement in %IF %THEN %DO block    *
***************************************************************************/

%macro assign_lib;

%if %sysfunc(fileexist(C:\Data\zipfiles)) and
    %sysfunc(fileexist(C:\Data\xptfiles)) and
    %sysfunc(fileexist(C:\Data\cptfiles)) and
    %sysfunc(fileexist(C:\Data\MySDS)) %then %do;
...
%end;
%else %do;
    %put WARNING: One or more folders are missing.;
%end;

%mend macro assign_lib;
...
%let xpt_files = h105 h113 h121 h129 h138 h147 h155 h163 h171 h181 h192;
